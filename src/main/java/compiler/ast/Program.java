...
public class Program {
    ...
    private ArrayList<StipulaContract> stipulaContracts = null;
    private ArrayList<Event> events = null;
    private ArrayList<Timer> timer;
    private String runningState = null;
    private boolean running = false;
    private final ArrayList<Object> lock = new ArrayList<>();
    ...
    public void addContract(StipulaContract c) {
        if (stipulaContracts == null) {
            stipulaContracts = new ArrayList<StipulaContract>();
        }
        stipulaContracts.add(c);
    }
    ...
    public ArrayList<StipulaContract> getContracts() {
        return stipulaContracts;
    }

    public StipulaContract getContract(int index) {
        for (int i = 0; i < stipulaContracts.size(); i++) {
            if (i == index) {
                return stipulaContracts.get(i);
            }
        }
        return null;
    }
    ...
    public void updateFieldsTypes(Map<Pair<String, Integer>, Type> typedVars) {
        for (Pair<String, Integer> pair : typedVars.keySet()) {
            for (Field f : fields) {
                if (f.getId().equals(pair.getFirst())) {
    ...
    public void updateFields(StipulaContract cnt) {
    ...
    public void updateParties(StipulaContract cnt) {
    ...
    public void updateFieldsAgreement(ArrayList<ArrayList<Pair<Field, String>>> values) {
        ...
        for (Pair<Field, String> el : tmp) {
            for (Field f : fields) {
                if (f.getId().equals(el.getFirst().getId())) {
                    DateUtils d = new DateUtils();
                    if (d.isValidDate(el.getSecond())) {
                        f.setValueStr(el.getSecond());
                        f.setType(new TimeType());
                    } else if (isNumeric(el.getSecond())) {
                        f.setValue((float) Double.parseDouble(el.getSecond()));
                        f.setType(new RealType());
                    } else if (el.getSecond().equals("true") || el.getSecond().equals("false")) {
                        f.setValueBool(Boolean.parseBoolean(el.getSecond()));
    ...
    public void updateAssets(StipulaContract cnt) {
    ...
    class DelayEvent extends TimerTask {
        ArrayList<Pair<Expression, ArrayList<Statement>>> toExecute;
        String initStateEvent;
        String endStateEvent;
        Event eventExec;
        TypeInference typesInf;
            ...
            synchronized (lock) {
                ...
                if (initStateEvent.equals(getRunningState())) {
                    ...
                    for (Pair<Expression, ArrayList<Statement>> pair : toExecute) {
                        ArrayList<Statement> stm = pair.getSecond();
                        eventExec.getContract().runStatements(true, typesInf, pair.getSecond());
                    }
    ...
    public void runProgram(TypeInference typeinferencer) throws InterruptedException {
        ...
        if (this.getAgreement() != null) {
            ...
            for (StipulaContract c : this.getContracts()) {
        ...
        while (flag) {
            ...
            switch (caseExec) {
                ...
                case 3:
                    ...
                    StipulaContract tmpContr = null;
                    for (int index = 0; index < this.getContracts().size(); index++) {
                        StipulaContract c = this.getContracts().get(index);
